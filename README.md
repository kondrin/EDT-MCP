...
**64 tools**, grouped by toolset. Full per-tool pages under [docs/tools/](docs/tools/).
...
| [`delete_metadata`](docs/tools/delete_metadata.md) | Delete a metadata node (object or member, including a FORM member - item / attribute / command / handler) addressed by a 1C full-name FQN, cascading the clea… |
...
| [`debug_status`](docs/tools/debug_status.md) | Report active debug launches: applicationId (real or synthetic 'attach:<name>'), launch configuration name/type, mode (debug/run), whether the target is curr… |
...
| [`resume`](docs/tools/resume.md) | Resume a suspended debug thread or all threads of a debug target. Pass threadId (from wait_for_break) or applicationId. With no arguments, resumes the single… |
